...
/// True when the GitHub login was replaced by the development bypass. Only ever true in a
/// Development-environment Debug build that explicitly asked for it.
/// </param>
public sealed record AdminAuthMode(bool DevBypass)
{
    /// <summary>
    /// Team the bypassed principal claims when no GitHub section supplies one. Lives here rather
    /// than on the handler so it is available to code that is compiled into Release builds too.
    /// </summary>
    public const string DevFallbackTeam = "dev-admin";
}
